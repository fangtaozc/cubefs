...
func (s *LcScanner) handleFile(dentry *proto.ScanDentry) {
	...
	case proto.OpTypeStorageClassHDD:
		...
		err = s.mw.UpdateExtentKeyAfterMigration(dentry.Inode, proto.OpTypeToStorageType(op), nil, dentry.LeaseExpire, delayDelMinute, dentry.Path, false, 0)
		if err != nil {
			if isSkipErr(err) {
				err = fmt.Errorf("skip (%v)", err)
				atomic.AddInt64(&s.currentStat.ExpiredSkipNum, 1)
	...
	case proto.OpTypeStorageClassEBS:
		...
		err = s.mw.UpdateExtentKeyAfterMigration(dentry.Inode, proto.OpTypeToStorageType(op), oek, dentry.LeaseExpire, delayDelMinute, dentry.Path, false, 0)
		if err != nil {
			if isSkipErr(err) {
				err = fmt.Errorf("skip (%v)", err)
				atomic.AddInt64(&s.currentStat.ExpiredSkipNum, 1)
